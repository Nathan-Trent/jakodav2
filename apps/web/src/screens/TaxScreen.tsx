...
import { useTaxSlice } from "@/lib/taxSlice";
...
export function TaxScreen() {
  ...
  const { tax, loading, refresh } = useTaxSlice();
  ...
  const obligations = useMemo<TaxObligation[]>(() => {
    ...
    if (!category) return [];
    return computeObligations({ asOf, profile: tax.profile, category, taxTypes: tax.taxTypes, rules: tax.rules, year: tax.year, month: tax.month });
  }, [tax, asOf]);
