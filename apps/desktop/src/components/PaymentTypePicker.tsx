 ...
 */
export function PaymentTypePicker({ value, onChange, disabled }: { value: PaymentType; onChange: (v: PaymentType) => void; disabled?: boolean }) {
  return (
    <div role="radiogroup" aria-label="How the customer paid" className="grid grid-cols-4 gap-1.5"
      onKeyDown={(e) => {
        if (disabled) return;
        const i = PAYMENT_TYPES.indexOf(value);
      ...
      {PAYMENT_TYPES.map((t) => {
        ...
        return (
          <button key={t} type="button" role="radio" aria-checked={on} tabIndex={on ? 0 : -1} disabled={disabled} onClick={() => onChange(t)}
