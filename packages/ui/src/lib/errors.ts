...
export function friendlyError(e: unknown): FriendlyError {
  ...
  if (/item_has_barcode|barcodes_one_generated_per_item|barcodes_one_per_item/i.test(raw)) {
    return { title: "This item already has a generated barcode", detail: "One generated label per item, so labels never disagree. Codes scanned from the pack are unlimited.", action: "Remove the existing generated barcode first, then generate a new one." };
  }
